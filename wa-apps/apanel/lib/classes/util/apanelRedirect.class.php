 ...
 *
 * Назначение:
 * - выполнять безопасный внутренний редирект в рамках текущего домена;
 * - определять target URL по цепочке return_url -> referer -> app_url;
 * - запрещать внешние и небезопасные URL.
 *
 * Зависимости:
 * - waRequest;
 * - waResponse;
 ...
 *
 * Инварианты:
 * - редирект выполняется только на локальный URL текущего домена;
 * - protocol-relative URL (//domain.com) запрещены;
 * - если target URL пустой, внешний или совпадает с текущим URL, используется URL приложения.
 *
 * Побочные эффекты:
 * - отправляет HTTP-редирект через wa()->getResponse()->redirect().
 *
 * Ошибки:
 * - некорректные URL не выбрасывают исключение, а приводят к безопасному fallback на URL приложения.
 */
final class apanelRedirect
{
    /**
    ...
    public static function redirectBack($target = ''): void
    {
        ...
        if (!self::isLocalUrl($target_url) || self::isCurrentUrl($target_url)) {
            $target_url = wa()->getAppUrl('apanel');
        }
    ...
    private static function resolveTargetUrl(string $url): string
    {
        ...
        return wa()->getAppUrl('apanel') . ltrim($url, '/');
    }
