 ...
 *
 * Назначение:
 * - преобразовывать строки между разными форматами записи;
 * - нормализовать входные строки из snake_case, kebab-case, camelCase, PascalCase,
 *   Title Case и смешанных форматов;
 * - использоваться для генерации кодов, ключей, имён методов, CSS/JS-идентификаторов
 *   и человекочитаемых заголовков;
 * - приводить строку к формату с первой буквой в верхнем регистре и остальными
 *   символами в нижнем регистре без изменения структуры исходной строки.
 *
 * Зависимости:
 * - waLocale;
 * - mbstring.
 *
 * Инварианты:
 * - все публичные методы принимают исходную строку и возвращают строку;
 * - методы toCamelCase(), toPascalCase(), toSnakeCase(), toKebabCase(),
 *   toUpperSnakeCase(), toTitleCase() используют внутреннюю нормализацию
 *   к набору слов в нижнем регистре;
 * - метод toCapitalizedCase() не разбивает строку на слова, не транслитерирует её
 *   и сохраняет исходные разделители;
 * - если после нормализации слов не осталось, возвращается пустая строка;
 * - при $drop_numeric_suffix=true удаляется только хвост вида "_15", "-15", ".15", "/15", " 15".
 *
 * Побочные эффекты:
 * - методы, использующие normalizeWords(), вызывают waLocale::transliterate()
 *   для приведения строки к латинице.
 *
 * Ошибки:
 * - исключения не перехватываются;
 * - пустая или невалидная после нормализации строка приводит к возврату пустой строки.
 */
final class apanelStringCase
{
    /**
