 ...
 * - control type строится как Apanel_ui_{snake_case};
 * - PHP-класс строится как apanelUi{PascalCase};
 * - шаблон строится как ApanelUi{PascalCase}.html;
 ...
 * - если шаблон указан, но файл не найден, исключение пробрасывается из apanelFetchHtml.
 */
final class apanelUi
{
    ...
    public static function getControl(string $type, string $name = '', array $params = []): string
    {
        ...
        $component_id = apanelStringCase::toSnakeCase($type);

        if ($component_id === '') {
            return '';
        }

        $component_suffix = apanelStringCase::toPascalCase($component_id);

        $params['control_type']       = 'Apanel_ui_' . $component_id;
        $params['component_id']       = $component_id;
        $params['component_class']    = 'apanelUi' . $component_suffix;
        $params['component_template'] = 'ApanelUi' . $component_suffix;
    ...
    public static function render(string $name, array $params = []): string
    {
        ...
        return apanelFetchHtml::getHtml([
            'file'   => "lib/classes/ui/templates/{$component_template}.html",
            'assign' => $params,
        ]);
    }
