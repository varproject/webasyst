        ...
        <div class="custom-p-24 summary-toolbar">
            ...
            <div>
                ...
                <div class="flexbox small custom-mb-4" v-if="summary!==false">
                    <span class="gray">[`Всего за отчетный период по условиям`]:</span>
                ...
                <div class="flexbox small custom-mb-4" v-if="summary!==false && summary.compares.sum">
                    <span class="gray">[`За предыдущий период по условиям`]:</span>
            ...
            <div v-if="summary!==false" style="margin-top: 0.5rem;">
                <ul class="paging smaller" v-if="filters.limit">
                    <li>
                ...
                <action-button
                        ...
                        action="tmp"
                        :run="false"
                        bclass="outlined smallest custom-ml-48 dark-gray"
                ...
                <action-button
                        ...
                        action="viewPromos"
                        :run="false"
                        bclass="outlined smallest custom-ml-24 dark-gray"
                        style="height: fit-content;"
                ></action-button>
                <action-button
                        @click="viewProducts()"
        ...
        <div class="custom-pl-24">
            <template v-if="summary!==false && runner!=='getSummary'">
                <table class="small zebra custom-mb-48" style="width: unset !important;">
                    ...
                    <tr v-for="(rec, idx) in summary.records">
                        <td class="large">
                            <span v-if="rec.slug" class="gray custom-mr-4" :title="getProductName(rec.slug)"><i
                                    class="far fa-check-square"></i></span>
                            <span v-else-if="!['order_payment','payout_bank'].includes(rec.type)" class="custom-mr-4"
                                  title="[`Не удалось определить продукт`]"><i
                                    class="far fa-question-circle"></i></span>

                            <span v-if="rec.type==='payout_bank'" :title="options.transaction_types[rec.type].name"><i
                                    class="fas fa-sign-out-alt"></i></span>
                            <span v-else-if="['order_payment_cancel', 'subscribe_cancel', 'purchase_cancel'].includes(rec.type)" style="color: var(--red)"
                                  :title="options.transaction_types[rec.type].name">
                                 <i class="fas fa-undo"></i></span>
                            <span v-else-if="rec.type==='purchase_upgrade'" style="color: var(--orange)"
                                  :title="options.transaction_types[rec.type].name">
                                 <i class="fas fa-external-link-square-alt"></i></span>
                            <span v-else-if="rec.type==='subscribe_month'" style="color: var(--blue)"
                                  :title="options.transaction_types[rec.type].name">
                                 <i class="far fa-calendar-alt"></i></span>
                            <span v-else-if="rec.type==='subscribe_year'" style="color: var(--blue)"
                                  :title="options.transaction_types[rec.type].name">
                                 <i class="fas fa-calendar-alt"></i></span>
                            <span v-else-if="rec.type==='subscribe'" style="color: var(--orange)"
                                  :title="options.transaction_types[rec.type].name">
                                <i class="far fa-calendar-check"></i></span>
                            <span v-else-if="rec.type==='lease'" style="color: var(--blue)"
                                  :title="options.transaction_types[rec.type].name">
                                 <i class="fas fa-cloud"></i></span>
                            <span v-else-if="rec.type==='purchase'" style="color: var(--green)"
                                  :title="options.transaction_types[rec.type].name">
                                <i class="far fa-money-bill-alt"></i></span>
                            <span v-else-if="rec.type==='order_payment'" style="color: var(--orange)"
                                  :title="options.transaction_types[rec.type].name">
                        ...
                        <td>{{ getDate('humanDateTime', rec.datetime) }}</td>
