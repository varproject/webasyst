...
// if sum of sku count doesn't equal count of product
// Notice: ignore sitaution p.count = NULL but sk.count != NULL because of SUM function. See next query
$sql = "
    SELECT p.id, p.count, SUM(sk.count) AS sk_count
    FROM `shop_product` p
    JOIN `shop_product_skus` sk ON p.id = sk.product_id
    WHERE sk.available = 1
    GROUP BY p.id
    HAVING
        (p.count IS NOT NULL AND sk_count IS NULL) OR
        (p.count != sk_count)
";
$model->exec($sql);
