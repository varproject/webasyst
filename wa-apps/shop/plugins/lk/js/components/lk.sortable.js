/**
 * lk.sortable.js — МОДУЛЬ: SORTABLE
 *
 * НАЗНАЧЕНИЕ
 ...
 *
 * ПУБЛИЧНЫЙ API
 * - lk.sortable.init([root]) — инициализация внутри контейнера root (по умолчанию document).
 *
 ...
 * ЗАВИСИМОСТИ
 * - SortableJS
 * - lk.helpers (опционально, для getCsrfToken)
...
; (function (lk, window, document) {
    ...
    var sortableModule = lk.sortable = lk.sortable || {};
    ...
    function saveOrder(container, ids, item) {
        ...
        var csrf = (lk.helpers && lk.helpers.getCsrfToken)
        ...
        window.fetch(url, {
            ...
            emit(container, 'lk:sortable:saved', {
                container: container,
                ids: ids,
                item: item,
                html: html,
            ...
            emit(container, 'lk:sortable:error', {
                container: container,
                ids: ids,
                item: item,
                error: error
