 ...
 *
 * ПУБЛИЧНЫЙ API
 * - lk.helpers.getCsrfToken() — получение CSRF токена.
 * - lk.helpers.initSlugGenerator(src, target) — привязка генератора к инпутам.
 *
 * ПРАВИЛА
...
; (function (window, document) {
    ...
    document.addEventListener('htmx:configRequest', function (evt) {
        ...
        var csrfToken = lk.helpers.getCsrfToken();
        if (!csrfToken) return;
    ...
    // =================================================================
    lk.helpers.initSlugGenerator = function (sourceSelector, targetSelector) {
