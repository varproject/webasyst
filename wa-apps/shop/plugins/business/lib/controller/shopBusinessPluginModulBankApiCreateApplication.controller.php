...
class shopBusinessPluginModulBankApiCreateApplicationController extends waJsonController
{
    public function execute()
    {
        ...
        if (ifempty($res['response']['status']) === 'ok') {
            $res['response']['message_html'] = <<<HTML
            <div class="custom-mt-16">
                <i class="fas fa-check-circle text-green" style="font-size:5rem;"></i>
                <h3>Заявка принята</h3>
                <p class="small">Заявка отправлена в МодульБанк. Пожалуйста, ожидайте ответа представителей банка.</p>
            </div>
HTML;
        }
