...
class shopBtobPluginAccessService extends shopBtobPluginChannelSettingsService
{
    public function resolve(int $channel_id, ?int $contact_id): array
    {
        ...
        $mode = ifset($params, 'btob_access_mode', 'public');
        ...
        if ($contact_id <= 0) {
            $guest_behavior = $mode === 'authorized' ? 'login' : ifset($params, 'btob_access_guest_behavior', 'login');
        ...
        if ($this->matches($contact_id, $this->getIdList(ifset($params, 'btob_access_deny_contact_ids', '')), $this->getIdList(ifset($params, 'btob_access_deny_category_ids', '')))) {
            return $this->deny('denied_list', $this->deniedBehavior($params));
        }

        $allow_contacts = $this->getIdList(ifset($params, 'btob_access_allow_contact_ids', ''));
        $allow_categories = $this->getIdList(ifset($params, 'btob_access_allow_category_ids', ''));
    ...
    {
        $behavior = ifset($params, 'btob_access_denied_behavior', 'page');
