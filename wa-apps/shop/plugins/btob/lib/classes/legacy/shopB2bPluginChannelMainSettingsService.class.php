...
class shopBtobPluginChannelMainSettingsService extends shopBtobPluginChannelSettingsService
{
    public function getViewData(array $channel): array
    {
        $params = ifset($channel, 'params', array());
        $settings = array(
            'route_key' => ifset($params, 'btob_main_route_key', ''),
            'frontend_from_root' => !empty($params['btob_main_frontend_from_root']),
            'frontend_custom_url' => ifset($params, 'btob_main_frontend_custom_url', 'btob'),
            'frontend_url' => ifset($params, 'btob_main_frontend_url', ''),
        );
    ...
    {
        $route_key = trim((string) ifset($input, 'btob_main_route_key', ifset($input, 'route_key', '')));
        $from_root = $this->getBool(ifset($input, 'btob_main_frontend_from_root', ifset($input, 'frontend_from_root', 0)));
        $custom_url = $this->normalizeSlug(ifset($input, 'btob_main_frontend_custom_url', ifset($input, 'frontend_custom_url', 'btob')), 'btob');
        $frontend_url = $from_root ? '*' : $custom_url . '/*';

        $data = array(
            'btob_version' => '2',
            'btob_main_route_key' => $route_key,
            'btob_main_frontend_from_root' => $from_root,
            'btob_main_frontend_custom_url' => $custom_url,
            'btob_main_frontend_url' => $frontend_url,
        ...
        if ($route) {
            $data += array(
                ...
                'btob_main_open_url' => $this->buildOpenUrl($data),
            );
        }
    ...
    public function validate(int $channel_id, array $settings): array
    {
        ...
        if (ifset($settings, 'btob_main_route_key', '') === '') {
            $errors[] = array('field' => 'data[params][btob_main_route_key]', 'error_description' => 'Выберите поселение Shop-Script.');
            return $errors;
        }

        $route = $this->parseRouteKey($settings['btob_main_route_key']);
        if (!$route) {
            $errors[] = array('field' => 'data[params][btob_main_route_key]', 'error_description' => 'Выбранное поселение Shop-Script не найдено.');
            return $errors;
        }

        if (ifset($settings, 'btob_main_frontend_url', '') === '') {
            $errors[] = array('field' => 'data[params][btob_main_frontend_custom_url]', 'error_description' => 'Укажите URL витрины.');
            return $errors;
        }

        if (!empty($settings['btob_main_frontend_from_root'])) {
            $errors[] = array(
                'field' => 'data[params][btob_main_frontend_from_root]',
                'error_description' => 'Открытие B2B-витрины от корня выбранного поселения сейчас конфликтует с основным маршрутом Shop-Script. Укажите URL витрины, например btob.'
            );
            return $errors;
        }
