...
HTML
    ...
    'receipt'                       => array(
        'value'        => false,
        'title'        => 'Фискализировать чеки через «ЮKassa»',
...
В остальных случаях оставьте вариант «Не передавать».',
    ...
    'credit_helper' => array(
        ...
        'description'  => <<<HTML
Вставьте эту строку в шаблон страницы или темы дизайна, чтобы показать на сайте условия оплаты в кредит через «ЮKassa».
<ul>
<li><code>\$amount_value</code> — сумма кредита.</li>
<li><code>"{$this->app_id}"</code> — идентификатор приложения.</li>
<li><code>"{$this->key}"</code> — идентификатор настроек плагина. Если у вас настроен только один способ оплаты через «ЮKassa», укажите значение <code>0</code>.</li>
